...
class DayOffCommandHandler(SlackCommandHandler):
    ...
    def handle(self, payload: RequestFreeDaysPayload):
        start_date = parse_formatted_date(payload.submission.start_day)
        end_date = parse_formatted_date(payload.submission.end_day)
        user = self.get_user_by_slack_user_id(payload.user.id)

        if start_date <= datetime.now().date:
            raise ValidationError('Holiday must start in the future', ['start_date'])

        if end_date < start_date:
            raise ValidationError('End date must not be lower than start date', ['end_date'])

        user_daysoff = self.dayoff_service.get_user_days_off(user.user_id)
        for dayoff in user_daysoff:
            if dayoff.start_date > start_date and start_date < dayoff.end_date:
                raise ValidationError('Day off must not start within other holiday')
            if dayoff.start_date > end_date and end_date < dayoff.end_date:
                raise ValidationError('Day off must not end within other holiday')            
